function [matches, negs,negCNNImage] = evaluateModel(...
  testImages, testBoxes, testBoxImages, w, hogCellSize, scales,last)
...
for i=1:numel(testImages)
  ...
  keep = boxsuppress(detections, scores, .5) ;
  keep = find(keep) ;
  keep = vl_colsubset(keep,15) ;
...
%   
if last == 1
  [x,y] = size(detections(:,sp));
  negatives = detections(:,sp);
  tempNeg = [];
  for j=1:y
      negBox = negatives(:,j);
      negBox(3:4) = negBox(3:4) - negBox(1:2);
      cropped = imresize(imcrop(cnnIm,negBox),[224,224]);
      tempNeg(:,:,:,j) = cropped;
  end
  negCNNImage = cat(4,negCNNImage,tempNeg);
end
  ...
  if nargout > 1
    ...
    detections = vl_colsubset(detections, 10) ;
